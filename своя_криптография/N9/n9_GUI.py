...
class Application:
    ...
    def create_menu(self):
        ...
        self.deep_menu.add_command(label="Файл", command=self.file)
    ...
    def encode(self):
        ...
        try:
            ...
            self.history_append('КОДИРОВАНИЕ')
    ...
    def decode(self):
        ...
        try:
            ...
            self.history_append('ДЕКОДИРОВАНИЕ')
            self.output_text.delete('1.0', END)
    ...
    def deep_encode(self):
        ...
        self.button_encode = Button(self.deep_encode, text='Кодировать', bd=2, command=self.start_deep_encode)
    ...
    def history(self):
        path = os.path.dirname(os.path.abspath(__file__))[0:] + '\history'
        os.system(f'start {path}')

    def history_append(self, do, deep=False):
        file_name = r'history/history.txt'
        with open(file_name, 'a', encoding='UTF-8') as f:
            key = self.key_entry.get()
            if deep == False:
                deep = 1
            data = time.strftime("%c")
            text = f'ДЕЙСТВИЕ: {do}  КЛЮЧ: {key}  ДАТА: {data}  ГЛУБИНА: {deep}\n'
            f.write(text)
